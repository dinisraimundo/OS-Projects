...
/*
  Function that is always reading from the client notification FIFO
*/
void* reads_notifs(void* arg){
  ...
  while(1){
    ...
    // In case we recieved SIGUSR1 in the host thread
    if(strcmp(key_buffer, "disconnect_sigma") == 0){
      ...
      // Closes the terminal for the client
      kill(getpid(), SIGKILL);
    }
...
int main(int argc, char* argv[]) {
  ...
  int connection = kvs_connect(req_pipe_path, resp_pipe_path, register_pipe_path, 
                              notif_pipe_path, &notif_fifo, &req_fifo, &resp_fifo);
  
  if(connection < 0){
    //  Because if connection == -2 the server could not open the register
    //  FIFO and we didn't open the other FIFOs
    if(connection == -1){
