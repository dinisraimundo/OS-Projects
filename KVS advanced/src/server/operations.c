...
// Add a client to the end of a linked list
...
// Subscribes a client to a certain key
int subscribe(const char * key, const char * client_id, int fd_resp_pipe, int fd_notif_pipe){
  ...
  // Write to client if subscription was successful
...
// Unsubscribes a client from a certain key
int unsubscribe(const char * key, const char * client_id, int fd_resp_pipe){
  ...
  // Write to client if unsubscription was successfull
  if (write_all(fd_resp_pipe, buffer, sizeof(buffer)) == -1) {
...
int disconnect(Client *client){
  ...
  // Go through clients keys and remove subscription
  while (keyNode != NULL){
...
// Delete every subscription in the server
int delete_subscriptions(Client *client){
  ...
  // Remove every subscription from the hashtable
