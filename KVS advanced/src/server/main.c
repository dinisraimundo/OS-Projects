...
/*
  This function handles the appearance of the signal SIGUSR1
*/
...
void handle_client_commands(Client * client){
  ...
  //  if the client processes were killed, without verification
  //  this would try to continue reading
  while (!killed){
...
// Function that threads run to exchange requests 
void* run_client(void* args) {
  ...
  // Block so the SIGUSR1 doesn't get acknowledged
  sigset_t sigset;
  sigemptyset(&sigset);
  ...
  while (1) {
    ...
    // Wait until there is a client to process (consumer semaphore) 
    sem_wait(&semPodeCons);
    pthread_mutex_lock(&buffer_mutex);

    // Access the client at the consptr index in the shared buffer
    Client* current_client = shared_buffer.clients[shared_buffer.consptr];

    if (current_client == NULL) {
      // Handle potential NULL client case and retry
